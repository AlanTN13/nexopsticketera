...
3. Radar Control Plane: registra el workspace interno y persiste únicamente estados/eventos recibidos por el callback server-to-server autenticado con HMAC. La autorización humana ocurre antes en la sesión, los RPC y RLS; el callback no acepta cookies ni IDs de empresa enviados por el navegador.

Las inserciones/actualizaciones de `public.users` que acompañan esas operaciones usan el cliente de la sesión y RLS. `service_role` no se usa para snapshots, tickets, comentarios, historial de soporte, empresas, métricas o Storage. En Radar queda acotado al puente de ejecución y nunca se expone al cliente.
