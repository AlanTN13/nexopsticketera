...
## EXECUTION RECEIPT + KNOWLEDGE DELTA — QA y preview productiva, 2026-09-21 22:45 UTC

**Correcciones integradas y desplegadas; portada/preview privadas verificadas en producción con contenido histórico. No se ejecutó otra búsqueda ni se aprobó/publicó una nota. Radar V1 editorial completo todavía no aceptado.**

- Código **2d25c6f333f629ce481013d6f25a9be2a1b5438f**, continuación sobre main de PR #75/#74 integradas. [CI 35663325414](https://github.com/AlanTN13/nexopsticketera/actions/runs/35663325414) SUCCESS: **340 tests / 61 suites**, typecheck, lint, export n8n y build Turbopack estándar PASS en Linux. Build local Webpack PASS; restricción de puerto local documentada abajo no afecta el CI/hosting.
- Workflow **BRbvcSEHpTIMpw63**, Published **ace8175d-0aaf-4118-b969-dcf6409da3a2** (`Radar QA coherente 2d25c6f`). Export posterior a publicación: diez nodos, tres Code nodes exactos a Git, mismos IDs/conexiones/settings/credenciales y pinData vacío. No cron ni nodo temporal conservado. El nuevo QA fue probado offline; no se atribuye una nueva ejecución OpenAI a esta versión.
- Primer deployment **dpl_FXRWUDshUzu2bqbTFJHCJkB6Fo5N** READY/Production. La prueba del compositor detectó `RADAR_WEB_PREVIEW_URL` configurada sólo en Preview/branch, ausente en Production. Se agregó únicamente la URL server-side `https://www.nexopstech.com/radar/preview`; firma existente y secretos intactos. Redeploy **dpl_9VmybX5obqiQ6pjgFBUpjWfbJLtm**, mismo SHA, READY/Production y alias **portal.nexopstech.com** verificados.
- **Evidencia viva de preview, con entrada histórica:** desde el compositor productivo del run **e7cf8e40-f2fa-44af-9cf4-93e7e2af5283**, POST `/api/radar/runs/e7cf8e40-f2fa-44af-9cf4-93e7e2af5283/preview` **200**, request **6ptps-1790030514818-a810cf54ffff**, deployment 9Vmy, 22:41:54.818 UTC. Logs obtenidos por CLI Vercel autorizada; el conector de logs devolvió 403 por su acceso, no por la aplicación.
- La ventana web productiva recibió el paquete por el puente existente y mostró **Vista previa privada · Sin publicar**, artículo completo, portada y botón de confirmación. Imagen observada/capturada desde esa ventana: **PNG 1600×900**, SHA256 **47ef42b5b655d83eeb6a744d96c0df9e83a76449152b83b87b36e3a1af6de161**, texto y acentos legibles. [Portada productiva de comprobación histórica](../execution-evidence/radar/2026-09-21-production-preview-historical-cover.png). La URL abierta sin opener no muestra la pieza. No se marcaron confirmaciones editoriales ni se pulsó publicación o aprobación humana.
- Supabase posterior: histórico e7cf8e40 sigue **failed**, updated_at **2026-09-04T12:57:07.744374Z**; rechazo real 480e1520 sigue **rejected**, updated_at **2026-09-21T21:35:04.721914Z**, usage y score null intactos. Última corrida sigue 21:34:14.522138Z. **Cero llamadas/gasto OpenAI adicional**, ledger **6/USD9** sin reset/refund. Ambos workspaces scheduler=false, autonomy_mode=review, publishingMode=review. Puente manual productivo sigue pendiente y autopublicación OFF.

**Knowledge Delta:** un PASS del modelo no basta cuando un control crítico es falso; el motivo por control y la normalización deben preceder portada y scoring. Validar firma/dimensiones PNG no detecta glifos ilegibles: fuente explícita empaquetada + inspección visual productiva sí lo resolvieron. Un env configurado sólo en Preview no está disponible tras merge a Production. La preview existente puede validarse con un run histórico admisible sin gastar proveedor, sin tocar el run rechazado ni fingir revisión humana.

**Estado:** implementado sí; integrado/mergeado sí; desplegado sí; transporte y rechazo QA vivos previos sí; preview/Sharp productivos con contenido histórico sí; E2E de una oportunidad nueva hasta READY_FOR_REVIEW con QA actualizado y revisión humana **no**; publicación productiva **no validada**; autónomo **no**. Próximo gate: autorización explícita para **una** reserva adicional USD1.50 (total USD10.50, sin reset/refund), luego una corrida acotada y revisión humana real si supera gates. No se amplió presupuesto. Sólo tras esa evidencia corresponde habilitar/comprobar el publicador manual existente; scheduler/autopublicación permanecen OFF.
