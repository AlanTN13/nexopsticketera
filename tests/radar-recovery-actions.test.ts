...
const m = vi.hoisted(() => ({ access: vi.fn(), admission: vi.fn(), find: vi.fn(), create: vi.fn(), reserve: vi.fn(), dispatch: vi.fn(), accept: vi.fn(), decide: vi.fn(), run: vi.fn(), schedule: vi.fn(), prefs: vi.fn() }));
...
beforeEach(() => { vi.resetAllMocks(); m.access.mockResolvedValue({ actor: { id: "actor" } }); m.find.mockResolvedValue(null); m.admission.mockResolvedValue({ allowed: false, code: "budget_exhausted", message: "Presupuesto agotado. Búsqueda no iniciada." }); });
describe("Radar Phase A admission", () => {
  ...
  it("rejects approval without a signed preview and rejects ineligible candidates even with a high score", async () => {
    m.run.mockResolvedValue({ id, workspaceId: "nexops", status: "review_pending", eligibility: "ELIGIBLE", candidate: { score: 99, qa: { verdict: "PASS" } } });
    ...
    m.run.mockResolvedValue({ id, workspaceId: "nexops", status: "review_pending", eligibility: "INELIGIBLE", candidate: { score: 99, qa: { verdict: "PASS" } } });
  ...
  it("accepts the authorized operator's signed exact-preview approval without publication", async () => {
    ...
    m.run.mockResolvedValue({ id, workspaceId: "nexops", status: "review_pending", eligibility: "ELIGIBLE", candidate: { score: 82, qa: { verdict: "PASS" } } });
    ...
    expect(result.error).toBeNull(); expect(m.decide).toHaveBeenCalledWith(expect.objectContaining({ actorId: "actor", decision: "approve" })); expect(m.dispatch).not.toHaveBeenCalled();
  });
