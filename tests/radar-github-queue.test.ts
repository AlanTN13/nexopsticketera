...
vi.mock("@/lib/radar-api-worker", () => ({ scheduleRadarApiWorker: schedule }));
...
describe("Radar API replaces queue while preserving historical request contract", () => {
  beforeEach(() => { vi.stubEnv("RADAR_API_ENABLED", "true"); vi.stubEnv("RADAR_API_PILOT_WORKSPACE_ID", "nexops"); vi.stubEnv("RADAR_API_PILOT_MAX_RUNS", "3"); vi.stubEnv("RADAR_OPENAI_MODEL", "gpt-5-mini"); vi.stubEnv("OPENAI_API_KEY", "test-only"); });
  ...
  it("schedules backend work without any GitHub queue mutation", async () => {
    const network = vi.fn(); vi.stubGlobal("fetch", network);
    expect(await dispatchRadarRun(baseInput)).toMatchObject({ externalRunId: `api:${runId}` }); expect(schedule).toHaveBeenCalledWith(runId); expect(network).not.toHaveBeenCalled();
  });
  it("never uses old queue credentials as silent fallback", async () => {
    vi.stubEnv("OPENAI_API_KEY", ""); vi.stubEnv("RADAR_QUEUE_GITHUB_TOKEN", "test-old-key"); vi.stubEnv("RADAR_ENGINE_CALLBACK_SECRET", "a".repeat(32));
