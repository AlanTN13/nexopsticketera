...
begin
  ...
  loop
    ...
    if selected_module = 'metrics' and item ? 'metaAdsEnabled' then
      if jsonb_typeof(item -> 'metaAdsEnabled') <> 'boolean' then
        raise exception 'La disponibilidad de Meta Ads no es válida.';
      end if;

      update public.company_modules
      set settings = jsonb_set(settings, '{metaAdsEnabled}', item -> 'metaAdsEnabled', true),
          updated_at = now()
      where company_id = target_company_id
        and module = 'metrics'
        and settings -> 'metaAdsEnabled' is distinct from item -> 'metaAdsEnabled';
    end if;
  end loop;
