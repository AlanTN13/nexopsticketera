...
export async function executeRadarEditorial(input: {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const safeVerdict = verdict === "PASS" && (!reviewEvidence || !allClaimsChecked) ? "REJECT" : verdict as "PASS" | "FIX" | "REJECT";
    ...
    const qa = { verdict: safeVerdict, reason: safeVerdict !== verdict ? "El crítico no aportó evidencia consultada para todas las afirmaciones." : reason! };
