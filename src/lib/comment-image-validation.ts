...
async function validateImages(files: File[], maxFiles: number, countError: string) {
  if (files.length > maxFiles) throw new Error(countError);
...
export async function validateCommentImages(files: File[]) {
  return validateImages(
    files,
    MAX_COMMENT_IMAGES,
    `Solo podés adjuntar hasta ${MAX_COMMENT_IMAGES} imágenes por mensaje.`,
  );
}

export async function validateTicketImages(files: File[]) {
  return validateImages(
    files,
    MAX_TICKET_IMAGES,
    `Solo podés adjuntar hasta ${MAX_TICKET_IMAGES} imágenes por ticket.`,
  );
}
