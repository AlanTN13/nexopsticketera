...
export function renderRadarCoverSvg(input: { title: string; topic: string; visualType: string; visualSubject?: string }, includeText = true) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="900" viewBox="0 0 1600 900"><rect width="1600" height="900" fill="#211245"/><path d="M980 0h620v900H980z" fill="#352061"/><g fill="#ffffff" font-family="Arial, Helvetica, sans-serif">${includeText ? '<text x="100" y="110" font-size="27" letter-spacing="5" fill="#c7baff">RADAR BY NEXOPS</text>' : ""}<rect x="100" y="150" width="130" height="8" rx="4" fill="#9ef3d4"/>${includeText ? title : ""}${motifs[input.visualType]}${includeText ? subject : ""}${includeText ? `<text x="100" y="800" font-size="28" fill="#c7baff">${escapeXml(input.topic.slice(0, 55))}</text>` : ""}</g></svg>`;
}
export async function renderRadarCover(input: Parameters<typeof renderRadarCoverSvg>[0]): Promise<RadarCover> {
  // Pango loads these files explicitly; SVG text alone silently falls back to
  // missing-glyph boxes on serverless hosts without installed fonts.
  process.env.FONTCONFIG_FILE ??= path.join(process.cwd(), "src/assets/radar-fonts/fonts.conf");
  const fonts = { regular: path.join(process.cwd(), "src/assets/radar-fonts/Lato-Regular.ttf"), bold: path.join(process.cwd(), "src/assets/radar-fonts/Lato-Bold.ttf") };
  await Promise.all(Object.values(fonts).map(file => access(file)));
  const blocks = [
    { text: "RADAR BY NEXOPS", size: 27, bold: false, color: "#c7baff", left: 100, top: 88, width: 780, height: 45 },
    { text: lines(input.title, 30, 5).join("\n"), size: 60, bold: true, color: "#ffffff", left: 100, top: 240, width: 780, height: 410 },
    { text: lines(input.visualSubject || input.topic, 29, 3).join("\n"), size: 23, bold: false, color: "#ffffff", left: 1050, top: 646, width: 460, height: 115 },
    { text: input.topic.slice(0, 55), size: 28, bold: false, color: "#c7baff", left: 100, top: 778, width: 780, height: 55 },
  ];
  const overlays = await Promise.all(blocks.map(async block => {
    const image = await sharp({ text: { text: `<span foreground="${block.color}">${escapeXml(block.text)}</span>`, font: `Lato ${block.bold ? "Bold " : ""}${block.size}`, fontfile: block.bold ? fonts.bold : fonts.regular, dpi: 72, rgba: true } }).png().toBuffer();
    const fitted = await sharp(image).resize({ width: block.width, height: block.height, fit: "inside", withoutEnlargement: true }).toBuffer();
    return { input: fitted, left: block.left, top: block.top };
  }));
  const png = await sharp(Buffer.from(renderRadarCoverSvg(input, false))).composite(overlays).png({ compressionLevel: 9 }).toBuffer();
