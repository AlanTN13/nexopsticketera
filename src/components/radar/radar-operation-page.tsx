...
import { radarPhase, radarResultTitle } from "@/lib/radar-presentation";
  ...
  return <div className="grid gap-6">
    ...
    {snapshot.availability !== "ready" || !settings ? <p role="status" className="rounded-xl border border-amber-200 bg-amber-50 p-5 text-sm">No se pudo verificar la disponibilidad de Radar. No se iniciará ninguna búsqueda.</p> : <>
      {view === "review" && <>
        ...
        {snapshot.runs[0] && !pending.some(run => run.id === snapshot.runs[0].id) && !activeRun && <section className="rounded-xl border border-slate-200 p-5"><h2 className="text-sm font-bold">Último resultado · {radarPhase(snapshot.runs[0])}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{snapshot.runs[0].errorMessage ?? snapshot.runs[0].resultReason ?? radarResultTitle(snapshot.runs[0])}</p><Link href={href("/historial", { run: snapshot.runs[0].id })} className="mt-3 inline-flex text-sm font-semibold text-[#4f35b5] underline">Ver resultado y motivo</Link></section>}
      </>}
      {view === "history" && <>
      ...
      {view === "configuration" && <>
        <section className="rounded-xl border border-slate-200 bg-slate-50 p-5"><h2 className="font-bold">Disponibilidad y piloto</h2><p className="mt-2 text-sm text-slate-600">{admission.message}</p>{admission.reservedUsd !== null && <p className="mt-2 text-sm">Reserva acumulada: USD {admission.reservedUsd.toFixed(2)} de USD {admission.maxUsd.toFixed(2)}. La reserva es un límite preventivo, no el costo facturado.</p>}<p className="mt-3 text-sm">Programación automática: <strong>{settings.schedulerEnabled ? "Configuración activa inesperada · operación bloqueada" : "No disponible · apagada"}</strong></p><p className="mt-2 text-sm">Autopublicación: <strong>{settings.preferences.publishingMode === "automatic" ? "Configuración activa inesperada · operación bloqueada" : "No disponible · apagada"}</strong></p><p className="mt-2 text-sm text-slate-600">Las preferencias de frecuencia no activan ejecuciones.</p></section>
