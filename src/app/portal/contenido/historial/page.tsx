...
export default async function ContentHistoryPage({ searchParams }: { searchParams: Promise<{ success?: string; wait?: string }> }) {
  ...
  return (
    <ContentShell context={context} active="history" title="Historial de recolección" description="Trazabilidad por corrida: origen, estado, cuentas procesadas y nuevos registros.">
      ...
      <SectionCard title="Operación semanal" description={`Programación ${context.workspace.scheduledEnabled ? "habilitada" : "desactivada hasta completar el smoke test oficial"}.`} tone="light" actions={context.canManage ? <PendingForm action={refreshContentAction}><input type="hidden" name="requestKey" value={randomUUID()} /><PendingSubmitButton disabled={context.connection?.status !== "connected"} idleLabel="Recolectar ahora" pendingLabel="Recolectando…" className="min-h-10 rounded-lg bg-indigo-700 px-4 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300" /></PendingForm> : null}>
