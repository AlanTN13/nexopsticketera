...
export default async function NewTicketPage() {
  ...
  if (!company || !hasModuleAccess(actor, company, "support", "operate")) redirect("/portal");
  const ticketCount = db.tickets.filter((ticket) => ticket.companyId === company.id).length;
  return <AppShell eyebrow="Portal NexOps · Soporte" title="Nuevo ticket" description="Contanos qué pasa y cómo afecta tu trabajo." tone="light" navigation={buildPortalNavigation({ active: "support", modules: getVisibleCompanyModules(actor, company), ticketCount })} actions={<NavButton href="/portal/soporte" label="Cancelar" muted tone="light" />}><SectionCard title="Crear solicitud" tone="light"><CreateTicketForm actor={actor} tone="light" compact /></SectionCard></AppShell>;
}
