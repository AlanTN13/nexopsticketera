...
import { ticketDetailPath, ticketListReturnPath } from "@/lib/routing";
...
export default async function BackofficeTicketDetail({ params, searchParams }: Props) {
  ...
  const canOperate = Boolean(company && hasModuleAccess(actor, company, "support", "operate"));
  const eligibleAssigneeIds = company && canOperate
    ? await getEligibleSupportAssigneeIds(company.id)
    : new Set<string>();
  const internalUsers = getInternalUsers(db).filter((user) => eligibleAssigneeIds.has(user.id));
  const listReturnPath = ticketListReturnPath(returnTo, company?.slug);
  const actionReturnPath = returnTo
    ? `${canonicalPath}?returnTo=${encodeURIComponent(listReturnPath)}`
    : canonicalPath;
  ...
  return <AppShell eyebrow="Backoffice · Ticket" title={`${ticket.code} · ${ticket.title}`} description="Gestión operativa y conversación del caso." tone="light"
    navigation={buildBackofficeNavigation({ actor, active: "tickets", ticketsHref: listReturnPath })}
    actions={<><NavButton href={listReturnPath} label="Volver a tickets" muted tone="light" /><LogoutClientForm tone="light" /></>}
  >
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
      <div className="grid gap-4">
        <SectionCard title="Conversación" description="Mensajes en orden cronológico. Las notas internas están claramente diferenciadas." tone="light">
          ...
          {canOperate ? <div className="mt-4 grid gap-3 border-t border-slate-200 pt-4 md:grid-cols-2"><div className="rounded-lg border border-slate-200 p-3"><AddCommentForm actor={actor} ticketId={ticket.id} returnPath={actionReturnPath} visibility="external" label="Responder al cliente" submitLabel="Enviar respuesta" tone="light" /></div><div className="rounded-lg border border-amber-200 bg-amber-50/40 p-3"><AddCommentForm actor={actor} ticketId={ticket.id} returnPath={actionReturnPath} visibility="internal" label="Agregar nota interna" submitLabel="Guardar nota interna" tone="light" /></div></div> : <p className="mt-4 border-t border-slate-200 pt-4 text-sm text-slate-600">Tu nivel permite ver Soporte, pero no responder ni agregar notas.</p>}
        </SectionCard>
      ...
      <div className="lg:sticky lg:top-4 lg:self-start"><SectionCard title="Gestión operativa" description="Estado, prioridad y asignación." tone="light">{canOperate ? <TicketWorkflowForm actor={actor} ticketId={ticket.id} assignedToId={ticket.assignedToId} status={ticket.status} priority={ticket.priority} internalAgents={internalUsers} returnPath={actionReturnPath} tone="light" /> : <p className="text-sm text-slate-600">Necesitás nivel operar para cambiar el workflow.</p>}</SectionCard></div>
    </div>
  </AppShell>;
}
