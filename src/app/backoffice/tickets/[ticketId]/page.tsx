...
export default async function BackofficeTicketDetail({ params }: Props) {
  ...
  const returnPath = `/backoffice/tickets/${ticket.id}`;
  ...
  return <AppShell eyebrow="Backoffice · Ticket" title={`${ticket.code} · ${ticket.title}`} description="Gestión operativa y conversación del caso." tone="light"
    navigation={[{ href: withActor("/backoffice/queue", actor.id), label: "Tickets", active: true }, { href: withActor("/backoffice/companies", actor.id), label: "Empresas" }, { href: withActor("/backoffice/users", actor.id), label: "Usuarios" }]}
    actions={<><NavButton href={withActor("/backoffice/queue", actor.id)} label="Volver a tickets" muted tone="light" /><LogoutClientForm tone="light" /></>}
  >
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
      <div className="grid gap-4">
        <SectionCard title="Conversación" description="Mensajes en orden cronológico. Las notas internas están claramente diferenciadas." tone="light">
          ...
          <div className="mt-4 grid gap-3 border-t border-slate-200 pt-4 md:grid-cols-2"><div className="rounded-lg border border-slate-200 p-3"><AddCommentForm actor={actor} ticketId={ticket.id} returnPath={returnPath} visibility="external" label="Responder al cliente" submitLabel="Enviar respuesta" tone="light" /></div><div className="rounded-lg border border-amber-200 bg-amber-50/40 p-3"><AddCommentForm actor={actor} ticketId={ticket.id} returnPath={returnPath} visibility="internal" label="Agregar nota interna" submitLabel="Guardar nota interna" tone="light" /></div></div>
        </SectionCard>
